...
namespace ug {
	static GraphicsType g_renderType = ug::GraphicsType::Unknown;
	static uint_t* g_methodsTable = NULL;
	...
	Status ug::init(GraphicsType _renderType)
	{
		...
		{
			DEBUG_PRINT("[!] Already Initialized Error");
			return AlreadyInitializedError;
		}

		if (_renderType != Unknown)
		{
			if (_renderType >= D3D9 && _renderType <= D3D12)
			{
				...
				windowClass.lpszClassName = UG_TEXT("UG");
				windowClass.hIconSm = NULL;

				::RegisterClassEx(&windowClass);

				HWND window = ::CreateWindow(windowClass.lpszClassName, UG_TEXT("UG DirectX Window"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);

				if (_renderType == D3D9)
				{
					HMODULE libD3D9;
					if ((libD3D9 = ::GetModuleHandle(UG_TEXT("d3d9.dll"))) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: libD3D9 = GetModuleHandle");
					...
					if ((Direct3DCreate9 = ::GetProcAddress(libD3D9, "Direct3DCreate9")) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: Direct3DCreate9 = GetProcAddress");
						return UnknownError;
					}
					...
					if ((direct3D9 = ((LPDIRECT3D9(__stdcall*)(uint32_t))(Direct3DCreate9))(D3D_SDK_VERSION)) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: direct3D9 = Direct3DCreate9");
						return UnknownError;
					}
					...
					if (direct3D9->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &displayMode) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: GetAdapterDisplayMode");
						return UnknownError;
					}
					...
					if (direct3D9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, window, D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_DISABLE_DRIVER_MANAGEMENT, &params, &device) < 0)
					{
						...
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateDevice");
						return UnknownError;
					}

					g_methodsTable = (uint_t*)::calloc(119, sizeof(uint_t));
					::memcpy(g_methodsTable, *(uint_t * *)device, 119 * sizeof(uint_t));
					...
					g_renderType = GraphicsType::D3D9;
				...
				else if (_renderType == GraphicsType::D3D10)
				{
					HMODULE libDXGI;
					HMODULE libD3D10;
					if ((libDXGI = ::GetModuleHandle(UG_TEXT("dxgi.dll"))) == NULL || (libD3D10 = ::GetModuleHandle(UG_TEXT("d3d10.dll"))) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: libDXGI = GetModuleHandle");
						return ModuleNotFoundError;
					}
					...
					if ((CreateDXGIFactory = ::GetProcAddress(libDXGI, "CreateDXGIFactory")) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateDXGIFactory = GetProcAddress");
						return UnknownError;
					}
					...
					if (((long(__stdcall*)(const IID&, void**))(CreateDXGIFactory))(__uuidof(IDXGIFactory), (void**)& factory) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateDXGIFactory");
						return UnknownError;
					}
					...
					if (factory->EnumAdapters(0, &adapter) == DXGI_ERROR_NOT_FOUND)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: EnumAdapters");
						return UnknownError;
					}
					...
					if ((D3D10CreateDeviceAndSwapChain = ::GetProcAddress(libD3D10, "D3D10CreateDeviceAndSwapChain")) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateDeviceAndSwapChain = GetProcAddress");
						return UnknownError;
					}
						...
						ID3D10Device * *))(D3D10CreateDeviceAndSwapChain))(adapter, D3D10_DRIVER_TYPE_HARDWARE, NULL, 0, D3D10_SDK_VERSION, &swapChainDesc, &swapChain, &device) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateDeviceAndSwapChain");
						return UnknownError;
					}

					g_methodsTable = (uint_t*)::calloc(116, sizeof(uint_t));
					::memcpy(g_methodsTable, *(uint_t * *)swapChain, 18 * sizeof(uint_t));
					::memcpy(g_methodsTable + 18, *(uint_t * *)device, 98 * sizeof(uint_t));
					...
					g_renderType = GraphicsType::D3D10;

					return Success;
				}
				else if (_renderType == GraphicsType::D3D11)
				{
					HMODULE libD3D11;
					if ((libD3D11 = ::GetModuleHandle(UG_TEXT("d3d11.dll"))) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: libD3D11 = GetModuleHandle");
						return ModuleNotFoundError;
					}
					...
					g_methodsTable = (uint_t*)::calloc(205, sizeof(uint_t));
					::memcpy(g_methodsTable, *(uint_t * *)swapChain, 18 * sizeof(uint_t));
					::memcpy(g_methodsTable + 18, *(uint_t * *)device, 43 * sizeof(uint_t));
					::memcpy(g_methodsTable + 18 + 43, *(uint_t * *)context, 144 * sizeof(uint_t));
					...
					g_renderType = GraphicsType::D3D11;

					return Success;
				}
				else if (_renderType == GraphicsType::D3D12)
				{
					HMODULE libDXGI;
					HMODULE libD3D12;
					if ((libDXGI = ::GetModuleHandle(UG_TEXT("dxgi.dll"))) == NULL || (libD3D12 = ::GetModuleHandle(UG_TEXT("d3d12.dll"))) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: libDXGI = GetModuleHandle");
					...
					if ((D3D12CreateDevice = ::GetProcAddress(libD3D12, "D3D12CreateDevice")) == NULL)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateDevice = GetProcAddress");
						return UnknownError;
					}
					...
					if (device->CreateCommandQueue(&queueDesc, __uuidof(ID3D12CommandQueue), (void**)& commandQueue) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateCommandQueue");
						return UnknownError;
					}
					...
					if (device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, __uuidof(ID3D12CommandAllocator), (void**)& commandAllocator) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateCommandAllocator");
						return UnknownError;
					}
					...
					if (device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAllocator, NULL, __uuidof(ID3D12GraphicsCommandList), (void**)& commandList) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateCommandList");
						return UnknownError;
					}
					...
					if (factory->CreateSwapChain(commandQueue, &swapChainDesc, &swapChain) < 0)
					{
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						DEBUG_PRINT("[!] UnknownError: CreateSwapChain");
						return UnknownError;
					}

					g_methodsTable = (uint_t*)::calloc(150, sizeof(uint_t));
					::memcpy(g_methodsTable, *(uint_t * *)device, 44 * sizeof(uint_t));
					::memcpy(g_methodsTable + 44, *(uint_t * *)commandQueue, 19 * sizeof(uint_t));
					::memcpy(g_methodsTable + 44 + 19, *(uint_t * *)commandAllocator, 9 * sizeof(uint_t));
					::memcpy(g_methodsTable + 44 + 19 + 9, *(uint_t * *)commandList, 60 * sizeof(uint_t));
					::memcpy(g_methodsTable + 44 + 19 + 9 + 60, *(uint_t * *)swapChain, 18 * sizeof(uint_t));
					...
					g_renderType = GraphicsType::D3D12;
			...
			else if (_renderType == GraphicsType::OpenGL)
			{
				HMODULE libOpenGL32;
				if ((libOpenGL32 = ::GetModuleHandle(UG_TEXT("opengl32.dll"))) == NULL)
				{
					DEBUG_PRINT("[!] ModuleNotFoundError: opengl32.dll");
					return ModuleNotFoundError;
				}
				...
				size_t size = UG_ARRAY_SIZE(methodsNames);

				g_methodsTable = (uint_t*)::calloc(size, sizeof(uint_t));

				for (int i = 0; i < size; i++)
				{
					g_methodsTable[i] = (uint_t)::GetProcAddress(libOpenGL32, methodsNames[i]);
				}
			...
			else if (_renderType == Vulkan)
			{
				HMODULE libVulkan;
				if ((libVulkan = GetModuleHandle(UG_TEXT("vulcan-1.dll"))) == NULL)
				{
					DEBUG_PRINT("[!] ModuleNotFoundError: vulcan-1.dll");
					return ModuleNotFoundError;
				}
				...
				for (int i = 0; i < size; i++)
				{
					g_methodsTable[i] = (uint_t)::GetProcAddress(libVulkan, methodsNames[i]);
				}
			...
			else if (_renderType == GDI)
			{
				DEBUG_PRINT("[?] Not Implemented");
				return NotImplemented;
			}
			else if (_renderType == GDIPLUS)
			{
				DEBUG_PRINT("[?] Not Implemented");
				return NotImplemented;
			}
			else if (_renderType == Auto)
			{
				GraphicsType type = Unknown;

				if (::GetModuleHandle(UG_TEXT("d3d9.dll")) != NULL)
				{
					type = D3D9;
					DEBUG_PRINT("[+] Detected d3d9 graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("d3d10.dll")) != NULL)
				{
					type = D3D10;
					DEBUG_PRINT("[+] Detected d3d10 graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("d3d11.dll")) != NULL)
				{
					type = D3D11;
					DEBUG_PRINT("[+] Detected d3d11 graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("d3d12.dll")) != NULL)
				{
					type = D3D12;
					DEBUG_PRINT("[+] Detected d3d12 graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("opengl32.dll")) != NULL)
				{
					type = OpenGL;
					DEBUG_PRINT("[+] Detected opengl graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("vulcan-1.dll")) != NULL)
				{
					type = Vulkan;
					DEBUG_PRINT("[+] Detected vulkan graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("gdi32.dll")) != NULL)
				{
					type = GDI;
					DEBUG_PRINT("[+] Detected gdi graphics library, initializing hook!");
				}
				else if (::GetModuleHandle(UG_TEXT("gdiplus.dll")) != NULL)
				{
					type = GDIPLUS;
					DEBUG_PRINT("[+] Detected gdiplus graphics library, initializing hook!");
				}
				else {
					type = Unknown;
					DEBUG_PRINT("[!] Could not find a supported Graphics library.");
				}
	...
	void shutdown()
	{
		if (g_renderType != Unknown)
		{
			...
			::free(g_methodsTable);
			g_methodsTable = NULL;
			g_renderType = Unknown;

			DEBUG_PRINT("[+] shutdown: Successfully shutdown all mh hooks, methods table and render type!");
		}
	}

	Status bind(uint16_t _index, void** _original, void* _function)
	{
		...
		if (g_renderType != Unknown)
		{
			...
			{
				DEBUG_PRINT("[!] bind (target: " << target << ", _index: " << _index << ", _original: " << _original << ", _function: " << _function << "): MH Error!");
				return MHError;
			}

			DEBUG_PRINT("[+] bind (target: " << target << ", _index: " << _index << ", _original: " << _original << ", _function: " << _function << "): Success!");
			return Success;
		}

		DEBUG_PRINT("[!] bind (_index: " << _index << ", _original: " << _original << ", _function: " << _function << "): Not Initialized Error!");
	...
	void unbind(uint16_t _index)
	{
		...
		if (g_renderType != GraphicsType::Unknown)
		{
			MH_DisableHook((void*)g_methodsTable[_index]);
			DEBUG_PRINT("[+] unbind (_index: " << _index << "): Successfully unbound method!");
		}
	}
